...
function change(id,d){const key=String(id);const x=cart.find(x=>String(x.id)===key);if(!x)return;x.q+=d;if(x.q<1)cart=cart.filter(y=>String(y.id)!==key);localStorage.bv_cart=JSON.stringify(cart);renderCart()}
