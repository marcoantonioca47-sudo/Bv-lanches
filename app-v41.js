...
function orderLabel(o){const n=Number(o?.orderNumber);return Number.isFinite(n)&&n>0?String(Math.trunc(n)).padStart(3,'0'):String(o?.id??'').slice(-5)}
...
let menuCategory='Lanches';function setMenuCategory(category,btn){menuCategory=category;document.querySelectorAll('[data-menu-category]').forEach(b=>b.classList.toggle('active',b===btn));renderProducts()}function renderProducts(){const box=$('products');if(box){const list=products.filter(p=>String(p.category||'Lanches').toLowerCase()===menuCategory.toLowerCase());box.innerHTML=list.map(p=>`<article class="product"><div class="photo">${p.emoji||'🍔'}</div><span class="categoryBadge">${p.category||'Lanches'}</span><h3>${p.name}</h3><p>${p.desc||''}</p><div class="price">${brl(p.price)}</div><button type="button" class="add" data-product-id="${String(p.id).replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;').replace(/>/g,'&gt;')}" onclick="return addFromButton(this,event)">Adicionar</button></article>`).join('')||'<div class="emptyMenu">Nenhum produto cadastrado nesta categoria.</div>'}renderManage()}
function add(id){const key=String(id??'').trim();const p=products.find(x=>String(x.id).trim()===key);if(!p){toast('Produto não encontrado. Atualize o cardápio.');return false}const i=cart.find(x=>String(x.id).trim()===key);if(i)i.q=Number(i.q||0)+1;else cart.push({...p,q:1});localStorage.setItem('bv_cart',JSON.stringify(cart));renderCart();toast('Produto adicionado ao carrinho');return false}
function addFromButton(btn,e){if(e){e.preventDefault();e.stopPropagation()}return add(btn?.dataset?.productId||'')}
document.addEventListener('click',e=>{const btn=e.target.closest('.cartQtyBtn');if(!btn)return;e.preventDefault();e.stopPropagation();change(btn.dataset.cartId,Number(btn.dataset.cartDelta)||0)},true);
function change(id,d){const key=String(id);const x=cart.find(x=>String(x.id)===key);if(!x)return;x.q+=d;if(x.q<1)cart=cart.filter(y=>String(y.id)!==key);localStorage.bv_cart=JSON.stringify(cart);renderCart()};window.change=change
function normalizeNeighborhoodName(v){return String(v||'').trim().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/\s+/g,' ')}
function fee(){const raw=$('bairro')?.value||'',b=normalizeNeighborhoodName(raw);if(b&&activeNeighborhoodFee.name===b&&activeNeighborhoodFee.value!==null)return Number(activeNeighborhoodFee.value)||0;if(b){const hit=Object.entries(config.bairroFees||{}).find(([name])=>normalizeNeighborhoodName(name)===b);if(hit)return Number(hit[1])||0}return Number(config.fee)||0}
let feeLookupTimer=null,activeNeighborhoodFee={name:'',value:null};
function refreshNeighborhoodFee(){clearTimeout(feeLookupTimer);const name=String($('bairro')?.value||'').trim();const normalized=normalizeNeighborhoodName(name);activeNeighborhoodFee={name:normalized,value:Number(config.fee)||0};renderCart();if(!name||!window.refreshDeliveryFeeForNeighborhood)return;feeLookupTimer=setTimeout(async()=>{const current=String($('bairro')?.value||'').trim();if(normalizeNeighborhoodName(current)!==normalized)return;const value=await window.refreshDeliveryFeeForNeighborhood(name);const finalValue=value!==null&&value!==undefined?Number(value)||0:Number(config.fee)||0;activeNeighborhoodFee={name:normalized,value:finalValue};if(value!==null&&value!==undefined){config.bairroFees=config.bairroFees||{};config.bairroFees[name.toLowerCase()]=finalValue;try{localStorage.bv_config=JSON.stringify(config)}catch(e){}}renderCart()},120)}
function bindNeighborhoodFee(){const el=$('bairro');if(!el||el.dataset.feeBound)return;el.dataset.feeBound='1';el.addEventListener('input',refreshNeighborhoodFee);el.addEventListener('change',refreshNeighborhoodFee)}function subtotal(){return cart.reduce((s,x)=>s+(Number(x.price)||0)*(Number(x.q)||0),0)}
function renderCart(){const sub=subtotal(),f=$('address')?.style.display!=='none'&&sub?fee():0,c=($('coupon')?.value||'').toUpperCase(),disc=c==='BV10'?sub*.1:c==='PRIMEIRA'?5:0,t=Math.max(0,sub+f-disc);if($('cart'))$('cart').innerHTML=cart.length?cart.map(x=>`<div class="line"><span>${x.q}x ${x.name}<br><small><button type="button" class="cartQtyBtn" data-cart-id="${String(x.id).replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;').replace(/>/g,'&gt;')}" data-cart-delta="-1">−</button> <button type="button" class="cartQtyBtn" data-cart-id="${String(x.id).replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;').replace(/>/g,'&gt;')}" data-cart-delta="1">+</button></small></span><b>${brl(x.price*x.q)}</b></div>`).join(''):'<p class="muted">Carrinho vazio.</p>';if($('sub'))$('sub').textContent=brl(sub);if($('fee'))$('fee').textContent=brl(f-disc);if($('total'))$('total').textContent=brl(t);['count','sideCount','quickCount'].forEach(id=>{if($(id))$(id).textContent=cart.reduce((s,x)=>s+x.q,0)})}
function mode(m,b){if($('address'))$('address').style.display=m==='entrega'?'block':'none';document.querySelectorAll('.tabs button').forEach(x=>x.classList.remove('active'));if(b)b.classList.add('active');if(m==='entrega'){bindNeighborhoodFee();refreshNeighborhoodFee()}renderCart()}function pay(p,b){localStorage.bv_payment=p;document.querySelectorAll('.pay button').forEach(x=>x.classList.remove('active'));if(b)b.classList.add('active');if($('troco'))$('troco').classList.toggle('hide',p!=='Dinheiro')}
