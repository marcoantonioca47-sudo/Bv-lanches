...
function fee(){const raw=$('bairro')?.value||'',b=normalizeNeighborhoodName(raw);if(b){const hit=Object.entries(config.bairroFees||{}).find(([name])=>normalizeNeighborhoodName(name)===b);if(hit)return Number(hit[1])||0}return Number(config.fee)||0}
let feeLookupTimer=null;
function refreshNeighborhoodFee(){clearTimeout(feeLookupTimer);const name=String($('bairro')?.value||'').trim();renderCart();if(!name||!window.refreshDeliveryFeeForNeighborhood)return;feeLookupTimer=setTimeout(async()=>{const current=String($('bairro')?.value||'').trim();if(current!==name)return;const value=await window.refreshDeliveryFeeForNeighborhood(name);if(value!==null&&value!==undefined){config.bairroFees=config.bairroFees||{};config.bairroFees[name.toLowerCase()]=Number(value)||0;try{localStorage.bv_config=JSON.stringify(config)}catch(e){};renderCart()}},180)}
