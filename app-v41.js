...
function refreshNeighborhoodFee(){clearTimeout(feeLookupTimer);const name=String($('bairro')?.value||'').trim();const normalized=normalizeNeighborhoodName(name);activeNeighborhoodFee={name:normalized,value:null};renderCart();if(!name||!window.refreshDeliveryFeeForNeighborhood)return;feeLookupTimer=setTimeout(async()=>{const current=String($('bairro')?.value||'').trim();if(normalizeNeighborhoodName(current)!==normalized)return;const value=await window.refreshDeliveryFeeForNeighborhood(name);if(value!==null&&value!==undefined){activeNeighborhoodFee={name:normalized,value:Number(value)||0};config.bairroFees=config.bairroFees||{};config.bairroFees[name.toLowerCase()]=Number(value)||0;try{localStorage.bv_config=JSON.stringify(config)}catch(e){};renderCart()}},120)}
