...
function renderCart(){const sub=subtotal(),f=$('address')?.style.display!=='none'&&sub?fee():0,c=($('coupon')?.value||'').toUpperCase(),disc=c==='BV10'?sub*.1:c==='PRIMEIRA'?5:0,t=Math.max(0,sub+f-disc);if($('cart'))$('cart').innerHTML=cart.length?cart.map(x=>`<div class="line"><span>${x.q}x ${x.name}<br><small><button onclick="change(${JSON.stringify(String(x.id))},-1)">−</button> <button onclick="change(${JSON.stringify(String(x.id))},1)">+</button></small></span><b>${brl(x.price*x.q)}</b></div>`).join(''):'<p class="muted">Carrinho vazio.</p>';if($('sub'))$('sub').textContent=brl(sub);if($('fee'))$('fee').textContent=brl(f-disc);if($('total'))$('total').textContent=brl(t);['count','sideCount','quickCount'].forEach(id=>{if($(id))$(id).textContent=cart.reduce((s,x)=>s+x.q,0)})}
