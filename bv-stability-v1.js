...
(()=>{ 
  ...
  window.orderItemsMarkup=items=>{const rows=Array.isArray(items)?items:[];const groups={};rows.forEach(i=>{const p=(window.products||[]).find(x=>norm(x.name)===norm(i.product_name||''));const cat=String(p?.category||'').trim();const key=cat==='Adicionais'?'ADICIONAIS':cat==='Bebidas'?'BEBIDAS':'ITENS';(groups[key]??=[]).push((Number(i.quantity)||1)+'x '+String(i.product_name||'Produto'));});const order=['ADICIONAIS','BEBIDAS','ITENS'];return order.filter(k=>groups[k]?.length).map(k=>'<div class="bvOrderItemGroup"><small>'+k+'</small><p>'+esc(groups[k].join(' • '))+'</p></div>').join('')||'<div class="bvOrderItemGroup"><small>ITENS</small><p>Itens do pedido</p></div>};
