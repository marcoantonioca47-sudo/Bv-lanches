...
(()=>{ 
  ...
  if(!$('bvFlavorStockStyle')){const st=document.createElement('style');st.id='bvFlavorStockStyle';st.textContent='.flavorStock{margin:11px 0 2px;padding:11px 12px;border:1px solid rgba(255,255,255,.08);border-radius:12px;background:linear-gradient(145deg,rgba(255,255,255,.055),rgba(255,255,255,.025));box-shadow:inset 0 1px 0 rgba(255,255,255,.035)}.flavorStock>span{display:block;margin-bottom:8px;color:#8f96a0;font-size:9px;font-weight:950;letter-spacing:1.2px;text-transform:uppercase}.flavorStockGrid{display:grid;grid-template-columns:1fr 1fr;gap:7px}.flavorStockItem{display:flex;align-items:center;justify-content:space-between;gap:8px;min-width:0;padding:8px 9px;border-radius:9px;background:rgba(0,0,0,.22);border:1px solid rgba(255,255,255,.055)}.flavorStockItem span{font-size:11px;font-weight:800;color:#dfe2e6;white-space:nowrap}.flavorStockItem strong{min-width:25px;text-align:center;padding:3px 6px;border-radius:6px;background:rgba(229,9,20,.12);border:1px solid rgba(229,9,20,.22);color:#ff5961;font-size:12px;font-weight:950}.flavorStockItem strong:empty{display:none}@media(max-width:600px){.flavorStock{padding:10px}.flavorStockGrid{gap:6px}.flavorStockItem{padding:8px 7px}.flavorStockItem span{font-size:10px}}';document.head.appendChild(st);}
  ...
  window.BV_STABILITY_VERSION='2026.09.26.343';
...
window.BV_ADD_PRODUCT_TO_CART=(p,flavor='')=>{
  ...
  window.renderCart=()=>{
    ...
    b.innerHTML=c.length?c.map(x=>{const p=(window.products||[]).find(y=>String(y.id)===String(x.productId||String(x.id).split('::')[0]));const category=String(x.category||p?.category||'Lanches');const categoryLabel=category==='Adicionais'?'ADICIONAIS':category==='Bebidas'?'BEBIDAS':'ITEM';return `<div class="cartRow"><div class="cartInfo"><small class="cartCategoryLabel">${categoryLabel}</small><b>${esc(x.name)}</b><small>${x.isPromotion?'Itens da promoção: '+(x.promotionItems||[]).map(i=>i.quantity+'x '+i.name).join(' • '):money(x.price)+' cada'}</small></div><div class="cartQty"><button type="button" onclick="change('${esc(x.id)}',-1)">−</button><strong>${Number(x.q)||0}</strong><button type="button" onclick="change('${esc(x.id)}',1)">+</button><button type="button" class="remove" onclick="removeFromCart('${esc(x.id)}')">×</button></div></div>`}).join(''):'<div class="emptyCart"><span>🛒</span><b>Seu carrinho está vazio</b><small>Escolha seus lanches no cardápio.</small><button type="button" onclick="showPage(\'cardapio\')">Ver cardápio</button></div>';
    ...
    if(v){
      try{
        ...
        if(productIds.length){
          ...
          if(pr.error)throw pr.error;
          const out=(pr.data||[]).find(p=>norm(p.category)==='bebidas'&&Number(p.stock||0)<=0);
          if(out){
