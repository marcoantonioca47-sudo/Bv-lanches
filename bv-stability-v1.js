...
(()=>{ 
  ...
  window.generatePromotionImage=async promotionId=>{try{const promo=(window.promotions||[]).find(x=>String(x.id)===String(promotionId));if(!promo)throw new Error('Promoção não encontrada.');const items=window.promotionItems?.[promo.id]||[];const ps=window.products||[];const rows=items.length?items:(promo.product_id?[{product_id:promo.product_id,quantity:1}]:[]);const names=rows.map(i=>{const p=ps.find(x=>String(x.id)===String(i.product_id));return Math.max(1,Number(i.quantity)||1)+'x '+(p?.name||'Produto')}).join(' • ');const escSvg=v=>String(v??'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');const wrap=(v,max)=>{const s=String(v||'');return s.length>max?s.slice(0,max-1)+'…':s};toast('🖼️ Criando imagem grátis da promoção...');const svg='<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="630" viewBox="0 0 1200 630"><defs><linearGradient id="bg" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#120000"/><stop offset=".55" stop-color="#4a0000"/><stop offset="1" stop-color="#080808"/></linearGradient></defs><rect width="1200" height="630" fill="url(#bg)"/><circle cx="1050" cy="80" r="250" fill="#ff3030" opacity=".12"/><circle cx="120" cy="650" r="260" fill="#ffd400" opacity=".08"/><text x="70" y="78" fill="#ffd400" font-family="Arial,sans-serif" font-size="34" font-weight="900">BV LANCHES</text><text x="70" y="170" fill="#fff" font-family="Arial,sans-serif" font-size="58" font-weight="900">'+escSvg(wrap(promo.name||'PROMOÇÃO',30))+'</text><text x="70" y="220" fill="#eee" font-family="Arial,sans-serif" font-size="28">'+escSvg(wrap(promo.description||'Oferta especial',55))+'</text><text x="70" y="280" fill="#fff" font-family="Arial,sans-serif" font-size="25" font-weight="700">'+escSvg(wrap(names,60))+'</text><text x="70" y="395" fill="#ff3030" font-family="Arial,sans-serif" font-size="72" font-weight="900">'+escSvg(money(Number(promo.promotional_price)||0))+'</text><text x="72" y="440" fill="#fff" font-family="Arial,sans-serif" font-size="22" font-weight="700">OFERTA ESPECIAL</text><rect x="70" y="475" width="230" height="58" rx="16" fill="#ffd400"/><text x="185" y="513" text-anchor="middle" fill="#120000" font-family="Arial,sans-serif" font-size="24" font-weight="900">PEÇA AGORA</text><text x="70" y="575" fill="#fff" opacity=".65" font-family="Arial,sans-serif" font-size="18">Promoção BV Lanches</text></svg>';const data='data:image/svg+xml;charset=UTF-8,'+encodeURIComponent(svg);promo.image_url=data;const upd=await sb.from('promotions').update({image_url:data}).eq('id',promo.id);if(upd.error)throw upd.error;window.renderHomePromoBanner?.();window.renderPromotionsAdmin?.();toast('✅ Imagem grátis criada com sucesso.');return true}catch(e){console.error('[BV PROMO IMAGE]',e);toast(e?.message||'Não foi possível criar a imagem.');return false}};
