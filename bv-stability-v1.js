...
(()=>{ 
  ...
  window.renderProducts=()=>{const b=$('products');if(!b)return;if(window.BV_MENU_CATEGORY==='Promocoes'){const now=Date.now(),active=(window.promotions||[]).filter(x=>x.active&&(!x.starts_at||new Date(x.starts_at).getTime()<=now)&&(!x.ends_at||new Date(x.ends_at).getTime()>=now)),ps=window.products||[];b.innerHTML=active.length?active.map(x=>{const items=window.promotionItems?.[x.id]||[],legacy=x.product_id?[{product_id:x.product_id,quantity:1}]:[],list=(items.length?items:legacy).map(i=>{const p=ps.find(y=>String(y.id)===String(i.product_id));return (i.quantity||1)+'x '+(p?.name||'Produto')}).join(' • ');return '<article class="productCard promotionCatalogCard"><div class="productImage"><span>🏷️</span></div><div class="promoCatalogBody"><h3>'+esc(x.name)+'</h3><p>'+esc(x.description||'Oferta especial do BV Lanches')+'</p><div class="promotionCatalogItems">'+esc(list)+'</div><div class="promotionCatalogPrice"><del>'+money(x.original_price||0)+'</del><strong>'+money(x.promotional_price||0)+'</strong></div><button type="button" class="promoCatalogButton" onclick="addPromotionToCart(\\''+esc(x.id)+'\\')">Adicionar promoção</button></div></article>'}).join(''):'<div class="emptyState"><span>🏷️</span><b>Nenhuma promoção ativa.</b><small>As promoções aparecerão aqui enquanto estiverem ativas.</small></div>';return;} 
    ...
    const now=Date.now();
    const activePromos=(window.promotions||[]).filter(x=>x.active&&(!x.starts_at||new Date(x.starts_at).getTime()<=now)&&(!x.ends_at||new Date(x.ends_at).getTime()>=now));
    const singlePromoByProduct={};
    activePromos.forEach(x=>{
      const items=window.promotionItems?.[x.id]||[];
      const legacy=x.product_id?[{product_id:x.product_id,quantity:1}]:[];
      const rows=items.length?items:legacy;
      if(rows.length===1&&Math.max(1,Number(rows[0].quantity)||1)===1){
        const promoPrice=Number(x.promotional_price);
        const productId=String(rows[0].product_id);
        if(Number.isFinite(promoPrice)) {
          const current=singlePromoByProduct[productId];
          if(!current||promoPrice<Number(current.promotional_price))singlePromoByProduct[productId]=x;
        }
      }
    });
    b.innerHTML=a.length?a.map(p=>{
      const fallback=p.category==='Bebidas'?'🥤':'🍔';
      const singlePromo=singlePromoByProduct[String(p.id)];
      const normalPrice=Number(p.price)||0;
      const promoPrice=singlePromo?Number(singlePromo.promotional_price):NaN;
      const hasPromo=!!singlePromo&&Number.isFinite(promoPrice)&&promoPrice<normalPrice;
      const price=hasPromo?promoPrice:normalPrice;
      ...
      const priceHtml=hasPromo?'<div class="promoPrice"><del>'+money(normalPrice)+'</del><b>'+money(promoPrice)+'</b></div>':'<b>'+money(normalPrice)+'</b>';
      return '<article class="productCard product">'+(hasPromo?'<div class="promoBadge">🔥 PROMOÇÃO</div>':'')+'<div class="productImage">'+media+'</div><div class="productInfo"><h3>'+esc(p.name)+'</h3><p>'+esc(p.description||'')+'</p><div class="productBottom">'+priceHtml+'<button type="button" onclick="addToCart(\\''+esc(p.id)+'\\')">+ Adicionar</button></div></div></article>';
  ...
  window.addToCart=id=>{const p=(window.products||[]).find(x=>String(x.id)===String(id));if(!p)return toast('Produto não encontrado.');const now=Date.now();const activePromos=(window.promotions||[]).filter(x=>x.active&&(!x.starts_at||new Date(x.starts_at).getTime()<=now)&&(!x.ends_at||new Date(x.ends_at).getTime()>=now));const promo=activePromos.find(x=>{const items=window.promotionItems?.[x.id]||[];const legacy=x.product_id?[{product_id:x.product_id,quantity:1}]:[];const rows=items.length?items:legacy;return rows.length===1&&String(rows[0].product_id)===String(id)&&Math.max(1,Number(rows[0].quantity)||1)===1&&Number.isFinite(Number(x.promotional_price))&&Number(x.promotional_price)<(Number(p.price)||0)});const unitPrice=promo?Number(promo.promotional_price):Number(p.price)||0;const cartId=promo?'promo-product:'+promo.id:String(p.id);const r=window.cart.find(x=>String(x.id)===cartId);if(r)r.q=(Number(r.q)||0)+1;else window.cart.push({id:cartId,name:p.name,price:unitPrice,q:1,...(promo?{isPromotion:true,isSingleProductPromotion:true,promotionId:promo.id,promotionItems:[{id:p.id,name:p.name,quantity:1}],normalPrice:Number(p.price)||0}:{} )});saveCart();toast(promo?'Produto adicionado com preço promocional.':'Produto adicionado ao pedido.')};
  ...
  window.renderCart=()=>{
    ...
    b.innerHTML=c.length?c.map(x=>`<div class="cartRow"><div class="cartInfo"><b>${esc(x.name)}</b><small>${x.isPromotion?(x.isSingleProductPromotion?money(x.price)+' cada · preço promocional (normal '+money(x.normalPrice)+')':'Itens da promoção: '+(x.promotionItems||[]).map(i=>i.quantity+'x '+i.name).join(' • ')):money(x.price)+' cada'}</small></div><div class="cartQty"><button type="button" onclick="change('${esc(x.id)}',-1)">−</button><strong>${Number(x.q)||0}</strong><button type="button" onclick="change('${esc(x.id)}',1)">+</button><button type="button" class="remove" onclick="removeFromCart('${esc(x.id)}')">×</button></div></div>`).join(''):'<div class="emptyCart"><span>🛒</span><b>Seu carrinho está vazio</b><small>Escolha seus lanches no cardápio.</small><button type="button" onclick="showPage(\\'cardapio\\')">Ver cardápio</button></div>';
  ...
  window.renderHomePromoBanner=()=>{
    ...
    track.innerHTML=active.map(x=>{const items=window.promotionItems?.[x.id]||[],legacy=x.product_id?[{product_id:x.product_id,quantity:1}]:[];const list=(items.length?items:legacy).map(it=>{const p=ps.find(y=>String(y.id)===String(it.product_id));return (it.quantity||1)+'x '+(p?.name||'Produto')}).join(' • ');return '<article class="homePromoSlide"><div><small class="homePromoKicker">🔥 OFERTA ESPECIAL</small><h3>'+esc(x.name)+'</h3><p>'+esc(x.description||'Aproveite enquanto durar!')+'</p><div class="homePromoItems">'+esc(list)+'</div></div><div class="homePromoPrice"><del>'+money(x.original_price||0)+'</del><strong>'+money(x.promotional_price||0)+'</strong><button type="button" onclick="showPage(\'cardapio\')">Ver oferta →</button></div></article>'}).join('');
