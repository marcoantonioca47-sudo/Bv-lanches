...
(()=>{ 
  ...
  window.generatePromotionImage=async promotionId=>{const promo=(window.promotions||[]).find(x=>String(x.id)===String(promotionId));if(!promo)return false;const items=window.promotionItems?.[promo.id]||[];const ps=window.products||[];const rows=(items.length?items:(promo.product_id?[{product_id:promo.product_id,quantity:1}]:[]));const resolved=rows.map(i=>{const p=ps.find(x=>String(x.id)===String(i.product_id));return {name:p?.name||'Produto',quantity:Math.max(1,Number(i.quantity)||1)}});toast('🖼️ Criando imagem grátis da promoção...');try{const w=1200,h=630,c=document.createElement('canvas');c.width=w;c.height=h;const x=c.getContext('2d');const g=x.createLinearGradient(0,0,w,h);g.addColorStop(0,'#120000');g.addColorStop(.55,'#4a0000');g.addColorStop(1,'#080808');x.fillStyle=g;x.fillRect(0,0,w,h);x.fillStyle='rgba(255,255,255,.07)';x.beginPath();x.arc(1030,90,240,0,Math.PI*2);x.fill();x.beginPath();x.arc(160,620,250,0,Math.PI*2);x.fill();x.fillStyle='#ffd400';x.font='900 34px Arial';x.fillText('BV LANCHES',70,75);x.fillStyle='#fff';x.font='900 58px Arial';const title=String(promo.name||'PROMOÇÃO').slice(0,30);x.fillText(title,70,165);x.fillStyle='#eee';x.font='500 28px Arial';const desc=String(promo.description||'Oferta especial').slice(0,52);x.fillText(desc,70,215);x.fillStyle='#fff';x.font='700 25px Arial';x.fillText(resolved.map(i=>i.quantity+'x '+i.name).join('  •  ').slice(0,55),70,275);x.fillStyle='#ff3030';x.font='900 70px Arial';x.fillText(money(Number(promo.promotional_price)||0),70,385);x.fillStyle='#fff';x.font='700 22px Arial';x.fillText('OFERTA ESPECIAL',72,430);x.fillStyle='#ffd400';x.font='900 26px Arial';x.fillText('PEÇA AGORA',70,510);x.fillStyle='rgba(255,255,255,.72)';x.font='500 18px Arial';x.fillText('Imagem criada automaticamente pelo BV Lanches',70,570);const data=c.toDataURL('image/jpeg',.88);promo.image_url=data;const upd=sb?await sb.from('promotions').update({image_url:data}).eq('id',promo.id):{error:null};if(upd.error)throw upd.error;window.renderHomePromoBanner?.();window.renderPromotionsAdmin?.();toast('✅ Imagem grátis criada com sucesso.');return true}catch(e){console.error('generatePromotionImageFree',e);toast(e?.message||'Não foi possível criar a imagem.');return false}};
