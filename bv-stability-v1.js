...
(()=>{ 
  ...
  window.BV_STABILITY_VERSION='2026.09.24.44';
  ...
  window.renderHomePromoBanner=()=>{
    ...
    track.innerHTML=active.map(x=>{const items=window.promotionItems?.[x.id]||[],legacy=x.product_id?[{product_id:x.product_id,quantity:1}]:[];const list=(items.length?items:legacy).map(it=>{const p=ps.find(y=>String(y.id)===String(it.product_id));return (it.quantity||1)+'x '+(p?.name||'Produto')}).join(' • ');return '<article class="homePromoSlide"'+(x.image_url?' style="background-image:linear-gradient(90deg,rgba(8,8,10,.96) 0%,rgba(8,8,10,.72) 45%,rgba(8,8,10,.18) 100%),url(\''+esc(x.image_url)+'\');background-size:cover;background-position:center;"':'')+'><div class="homePromoCopy"><small class="homePromoKicker">🔥 OFERTA ESPECIAL</small><h3>'+esc(x.name)+'</h3><p>'+esc(x.description||'Aproveite enquanto durar!')+'</p><div class="homePromoItems">'+esc(list)+'</div></div><div class="homePromoPrice"><del>'+money(x.original_price||0)+'</del><strong>'+money(x.promotional_price||0)+'</strong><button type="button" onclick="showPage(\'cardapio\')">Ver oferta →</button></div></article>'}).join('');
  ...
  window.generatePromotionImage=async promotionId=>{try{const promo=(window.promotions||[]).find(x=>String(x.id)===String(promotionId));if(!promo)throw new Error('Promoção não encontrada.');if(!window.promotionItems?.[promo.id]&&window.BV_REFRESH_PROMOTIONS)await window.BV_REFRESH_PROMOTIONS();const items=window.promotionItems?.[promo.id]||[];const ps=window.products||[];const rows=items.length?items:(promo.product_id?[{product_id:promo.product_id,quantity:1}]:[]);const products=rows.map(i=>({p:ps.find(x=>String(x.id)===String(i.product_id)),q:Math.max(1,parseInt(i.quantity,10)||1)})).filter(x=>x.p);if(!products.length)throw new Error('Não encontrei os produtos desta promoção.');toast('🖼️ Criando imagem com as fotos e quantidades...');const loadImg=url=>new Promise(resolve=>{if(!url)return resolve(null);const im=new Image();im.crossOrigin='anonymous';im.onload=()=>resolve(im);im.onerror=()=>resolve(null);im.src=url});const imgs=await Promise.all(products.map(x=>loadImg(x.p.image_url)));const c=document.createElement('canvas');c.width=1200;c.height=630;const ctx=c.getContext('2d');const bg=ctx.createLinearGradient(0,0,1200,630);bg.addColorStop(0,'#120000');bg.addColorStop(.55,'#500000');bg.addColorStop(1,'#090909');ctx.fillStyle=bg;ctx.fillRect(0,0,1200,630);ctx.textAlign='center';ctx.fillStyle='#ffd400';ctx.font='900 30px Arial';ctx.fillText('BV LANCHES',600,45);ctx.fillStyle='#fff';ctx.font='900 44px Arial';ctx.fillText(String(promo.name||'PROMOÇÃO').slice(0,28),600,92);const n=products.length, gap=20, cardW=n===1?430:n===2?360:Math.min(285,(1100-gap*(n-1))/n), total=cardW*n+gap*(n-1), left=(1200-total)/2;for(let k=0;k<n;k++){const x=left+k*(cardW+gap),y=120;ctx.fillStyle='#fff';ctx.beginPath();ctx.roundRect(x,y,cardW,355,22);ctx.fill();ctx.save();ctx.beginPath();ctx.roundRect(x+10,y+10,cardW-20,220,16);ctx.clip();if(imgs[k]){const im=imgs[k],scale=Math.max((cardW-20)/im.width,220/im.height),w=im.width*scale,h=im.height*scale;ctx.drawImage(im,x+10+(cardW-20-w)/2,y+10+(220-h)/2,w,h)}else{ctx.fillStyle='#eeeeee';ctx.fillRect(x+10,y+10,cardW-20,220);ctx.font='60px Arial';ctx.textAlign='center';ctx.fillStyle='#777';ctx.fillText('🍔',x+cardW/2,y+140)}ctx.restore();ctx.fillStyle='#e40000';ctx.beginPath();ctx.roundRect(x+18,y+18,82,48,20);ctx.fill();ctx.fillStyle='#fff';ctx.font='900 24px Arial';ctx.textAlign='center';ctx.fillText(products[k].q+'x',x+59,y+51);ctx.fillStyle='#151515';ctx.font='900 '+(n>2?'20':'24')+'px Arial';ctx.fillText(String(products[k].p.name||'Produto').slice(0,n>2?18:24),x+cardW/2,y+275);ctx.fillStyle='#666';ctx.font='600 16px Arial';ctx.fillText('Quantidade: '+products[k].q,x+cardW/2,y+305)}ctx.fillStyle='#ff3030';ctx.font='900 52px Arial';ctx.textAlign='center';ctx.fillText(money(Number(promo.promotional_price)||0),600,530);ctx.fillStyle='#ffd400';ctx.font='900 21px Arial';ctx.fillText('OFERTA ESPECIAL • PEÇA AGORA',600,570);let data;try{data=c.toDataURL('image/jpeg',.88)}catch(e){throw new Error('A foto de algum produto bloqueou a criação da imagem. Verifique as imagens dos produtos.')}const upd=await sb.from('promotions').update({image_url:data}).eq('id',promo.id);if(upd.error)throw upd.error;promo.image_url=data;window.renderHomePromoBanner?.();window.renderPromotionsAdmin?.();toast('✅ Imagem criada com '+products.reduce((s,x)=>s+x.q,0)+' unidade(s), usando as fotos dos produtos.');return true}catch(e){console.error('[BV PROMO IMAGE]',e);toast('❌ '+(e?.message||'Não foi possível criar a imagem.'));return false}};
  window.renderPromotionsAdmin=()=>{const ps=(window.products||[]).filter(x=>x.active!==false);const sel=$('promoProduct');if(sel)sel.innerHTML='<option value="">Selecione o produto</option>'+ps.map(p=>'<option value="'+esc(p.id)+'">'+esc(p.name)+' — '+money(p.price)+'</option>').join('');const b=$('promotionsManage');if(!b)return;const a=window.promotions||[];const itemBox=$('promoItems');if(itemBox&&!itemBox.children.length)window.addPromoItemRow?.();b.innerHTML=a.length?a.map(x=>{const items=window.promotionItems?.[x.id]||[];const legacy=x.product_id?[{product_id:x.product_id,quantity:1}]:[];const list=(items.length?items:legacy).map(i=>{const p=ps.find(y=>String(y.id)===String(i.product_id));return (i.quantity||1)+'x '+esc(p?.name||'Produto')}).join(' + ');return '<article class="promotionAdminCard"><div class="promoAdminMain">'+(x.image_url?'<img class="promoAdminThumb" src="'+esc(x.image_url)+'" alt="Imagem da promoção">':'<div class="promoAdminThumb promoAdminThumbEmpty">🤖</div>')+'<div><span class="promoBadge">🔥 PROMOÇÃO</span><h3>'+esc(x.name)+'</h3><p>'+esc(x.description||'')+'</p><small>'+list+'</small></div></div><div class="promoAdminPrice"><del>'+money(x.original_price||0)+'</del><b>'+money(x.promotional_price||0)+'</b></div><div class="promoActions"><button type="button" onclick="window.generatePromotionImage(this.getAttribute(&quot;data-promotion-id&quot;))" data-promotion-id="'+esc(x.id)+'">🤖 '+(x.image_url?'Refazer imagem':'Gerar imagem')+'</button><button type="button" onclick="togglePromotion(\''+esc(x.id)+'\','+(!x.active)+')">'+(x.active?'Desativar':'Ativar')+'</button><button type="button" onclick="removePromotion(\''+esc(x.id)+'\')">Excluir</button></div></article>'}).join(''):'<div class="emptyState"><span>🏷️</span><b>Nenhuma promoção cadastrada.</b><small>Cadastre a primeira oferta abaixo.</small></div>'};
