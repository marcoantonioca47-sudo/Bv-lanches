...
(()=>{ 
  ...
  window.generatePromotionImage=async promotionId=>{const promo=(window.promotions||[]).find(x=>String(x.id)===String(promotionId));if(!promo)return false;const items=window.promotionItems?.[promo.id]||[];const ps=window.products||[];const resolved=(items.length?items:(promo.product_id?[{product_id:promo.product_id,quantity:1}]:[])).map(i=>{const p=ps.find(x=>String(x.id)===String(i.product_id));return {name:p?.name||'Produto',quantity:Math.max(1,Number(i.quantity)||1)}});toast('🤖 Criando imagem da promoção com IA...');try{const r=await sb.functions.invoke('generate-promotion-image',{body:{promotion_id:promo.id,name:promo.name,description:promo.description||'',promotional_price:Number(promo.promotional_price)||0,items:resolved}});if(r.error||!r.data?.ok)throw new Error(r.error?.message||r.data?.error||'Não foi possível gerar a imagem.');promo.image_url=r.data.image_url;window.renderHomePromoBanner?.();window.renderPromotionsAdmin?.();toast('✅ Imagem da promoção criada com IA.');return true}catch(e){console.error('generatePromotionImage',e);toast(e?.message||'Não foi possível gerar a imagem da promoção.');return false}};
