...
(()=>{ 
  ...
  window.BV_STABILITY_VERSION='2026.09.24.82';
  ...
  window.addProduct=async e=>{e.preventDefault();if(window.BV_ROLE!=='administrador'&&window.BV_ROLE!=='admin')return toast('Acesso restrito ao administrador.');if(!sb)return toast('Banco de dados indisponível. Recarregue a página.');const n=$('productName')?.value.trim(),p=Number(String($('productPrice')?.value||'').replace(',','.')),c=$('productCategory')?.value,d=$('productDesc')?.value.trim();if(!n||!Number.isFinite(p)||p<0||!c)return toast('Preencha nome, valor e categoria.');const btn=e.target.querySelector('.formSave');if(btn){btn.disabled=true;btn.textContent='Cadastrando...'}try{const r=await sb.from('products').insert({name:n,price:p,category:c,description:d||'',active:true}).select('id').single();if(r.error)throw r.error;e.target.reset();window.closeProductForm();await window.BV_REFRESH_PRODUCTS();toast('Produto cadastrado com sucesso.')}catch(err){console.error('Cadastro de produto:',err);toast('Erro ao cadastrar produto: '+(err?.message||'Verifique os dados e tente novamente.'))}finally{if(btn){btn.disabled=false;btn.textContent='✓ Cadastrar produto'}}};
