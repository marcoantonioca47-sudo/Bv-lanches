...
(()=>{ 
  ...
  window.renderProducts=async()=>{const b=$('products');if(!b)return;const refri=(window.products||[]).find(x=>norm(x?.name)==='refri 2l');if(refri)await window.BV_REFRESH_FLAVOR_STOCKS?.();if(window.BV_MENU_CATEGORY==='Promocoes'){const now=Date.now(),active=(window.promotions||[]).filter(x=>x.active&&(!x.starts_at||new Date(x.starts_at).getTime()<=now)&&(!x.ends_at||new Date(x.ends_at).getTime()>=now)),ps=window.products||[];b.innerHTML=active.length?active.map(x=>{const items=window.promotionItems?.[x.id]||[],legacy=x.product_id?[{product_id:x.product_id,quantity:1}]:[],resolvedPromo=(items.length?items:legacy).map(i=>{const p=ps.find(y=>String(y.id)===String(i.product_id));return {p,q:Math.max(1,Number(i.quantity)||1)}}).filter(v=>v.p);const grouped=['Lanches','Bebidas','Adicionais'].map(cat=>{const rows=resolvedPromo.filter(v=>String(v.p.category||'Lanches')===cat);if(!rows.length)return '';return '<div class="bvOrderItemGroup"><small>'+cat.toUpperCase()+'</small><p>'+esc(rows.map(v=>v.q+'x '+v.p.name).join(' • '))+'</p></div>'}).join('');return '<article class="productCard promotionCatalogCard"><div class="productImage"><span>🏷️</span></div><div class="promoCatalogBody"><h3>'+esc(x.name)+'</h3><p>'+esc(x.description||'Oferta especial do BV Lanches')+'</p><div class="promotionCatalogItems">'+grouped+'</div><div class="promotionCatalogPrice"><del>'+money(x.original_price||0)+'</del><strong>'+money(x.promotional_price||0)+'</strong></div><button type="button" class="promoCatalogButton" onclick="addPromotionToCart(\''+esc(x.id)+'\')">Adicionar promoção</button></div></article>'}).join(''):'<div class="emptyState"><span>🏷️</span><b>Nenhuma promoção ativa.</b><small>As promoções aparecerão aqui enquanto estiverem ativas.</small></div>';return;}
    ...
    b.innerHTML=a.length?a.map(p=>{
      ...
      const stockHtml=isRefri2L?'<div class="catalogStock flavorStock"><span>ESTOQUE POR SABOR</span><div class="flavorStockGrid"><div class="flavorStockItem"><span>🥤 Guaraná</span><strong>'+gs+'</strong></div><div class="flavorStockItem"><span>🍊 Laranja</span><strong>'+ls+'</strong></div></div></div>':(isRefrigerante?'<div class="catalogStock '+(outOfStock?'out':'')+'"><span>ESTOQUE</span><strong>'+stock+'</strong>'+(outOfStock?'<em>ESGOTADO</em>':'<small>disponível</small>')+'</div>':'');
...
window.BV_ADD_PRODUCT_TO_CART=(p,flavor='')=>{
  ...
  window.orderItemsMarkup=items=>{let rows=[];if(Array.isArray(items)){rows=items.map(i=>({product_id:i?.product_id||i?.productId||'',name:String(i?.product_name||i?.name||'Produto'),quantity:Math.max(1,Number(i?.quantity)||1)}));}else{const raw=String(items??'').trim();if(raw){rows=raw.split(/\\s*(?:•|,|\\n)\\s*/).map(part=>{const m=part.match(/^([0-9]+)x\\s*(.+)$/i);return{name:String(m?m[2]:part).trim(),quantity:Math.max(1,Number(m?m[1]:1)||1)}}).filter(x=>x.name);}}const groups={Lanches:[],Bebidas:[],Adicionais:[]};rows.forEach(i=>{const name=i.name||'Produto';const baseName=name.split(' — ')[0].trim();const p=(window.products||[]).find(x=>(i.product_id&&String(x.id)===String(i.product_id))||norm(x?.name)===norm(baseName));const cat=String(p?.category||'').trim().toLowerCase();const key=cat==='bebidas'?'Bebidas':cat==='adicionais'?'Adicionais':'Lanches';groups[key].push(i.quantity+'x '+name)});return ['Lanches','Bebidas','Adicionais'].filter(k=>groups[k].length).map(k=>'<div class="bvOrderItemGroup"><small>'+k.toUpperCase()+'</small><p>'+esc(groups[k].join(' • '))+'</p></div>').join('')||'<div class="bvOrderItemGroup"><small>LANCHES</small><p>Itens do pedido</p></div>'};
...
window.BV_TRACKING_REALTIME=null;
  ...
  window.renderTracking=o=>{const b=$("trackingResult");if(!b)return;o=o||(window.orders||[]).find(x=>String(x.id)===String(localStorage.getItem('bv_track_id')))||(window.orders||[]).find(x=>!['entregue','cancelado'].includes(String(x.rawStatus||'').toLowerCase()));if(!o){b.innerHTML='<div class="trackingEmpty"><span>📦</span><h3>Nenhum pedido selecionado</h3><p>Informe um pedido ou veja seu último pedido para acompanhar o status.</p></div>';return}const rawStatus=String(o.rawStatus||o.status||'').trim().toLowerCase();const displayStatus=String(status[rawStatus]||o.status||rawStatus||'Pedido recebido');o.rawStatus=rawStatus;window.BV_PIX_CODE=o.pixQrCode||'';const isPix=String(o.payment||'').toLowerCase()==='pix';const paid=String(o.paymentStatus||'').toLowerCase()==='pago';const statusClass=rawStatus.replace('_','-')||'recebido';const itemHtml=window.orderItemsMarkup?window.orderItemsMarkup(o.items||''):esc(o.items||'Itens do pedido');const pixBox=isPix&&!paid?'<div class="trackingPix waiting"><b>PIX — aguardando pagamento</b><small>Pague pelo QR Code ou use o Pix Copia e Cola.</small>'+(o.pixQrCodeBase64?'<img src="data:image/png;base64,'+esc(o.pixQrCodeBase64)+'" alt="QR Code PIX">':'')+(o.pixQrCode?'<button type="button" onclick="copyPixCode()">📋 Copiar Pix Copia e Cola</button>':'')+'</div>':(isPix&&paid?'<div class="trackingPix paid"><b>✅ PIX confirmado</b><small>Pagamento recebido. Pedido liberado para preparo.</small></div>':'');const steps=['recebido','em_preparo','em_producao','saiu_entrega','entregue'];const idx=steps.indexOf(rawStatus);b.innerHTML='<div class="trackingCard trackingCardFixed"><div class="trackingCardTop"><div><small>PEDIDO</small><h3>#'+esc(window.orderLabel(o))+'</h3></div><span class="trackingStatusPill '+statusClass+'">'+esc(displayStatus)+'</span></div><div class="trackingProgress"><div class="trackingProgressStep '+(idx>=0?'done':'')+'"><span>1</span><small>Recebido</small></div><div class="trackingProgressStep '+(idx>=1?'done':'')+'"><span>2</span><small>Preparo</small></div><div class="trackingProgressStep '+(idx>=3?'done':'')+'"><span>3</span><small>Entrega</small></div><div class="trackingProgressStep '+(idx>=4?'done':'')+'"><span>4</span><small>Entregue</small></div></div><div class="trackingCardSection"><small>ITENS DO PEDIDO</small><div class="trackingItems">'+itemHtml+'</div></div><div class="trackingCardTotal"><span>Total</span><strong>'+money(o.total)+'</strong></div>'+(o.payment?'<div class="trackingPayment"><span>Pagamento</span><b>'+esc(o.payment)+'</b></div>':'')+pixBox+'</div>';window.startPixStatusPolling(o)};
...
})();<style data-bv-tracking-fix>.trackingCardFixed{padding:18px!important;overflow:hidden}.trackingCardFixed .trackingCardTop{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;padding-bottom:14px;border-bottom:1px solid rgba(255,255,255,.08)}.trackingCardFixed .trackingCardTop small,.trackingCardFixed .trackingCardSection>small{font-size:9px;font-weight:950;letter-spacing:.12em;color:#ff5b64}.trackingCardFixed .trackingCardTop h3{margin:4px 0 0;font-size:25px}.trackingStatusPill{display:inline-flex;align-items:center;justify-content:center;min-height:32px;padding:6px 10px;border-radius:999px;font-size:11px;font-weight:950;white-space:nowrap;background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.1)}.trackingStatusPill.recebido{color:#ffd166}.trackingStatusPill.em-preparo,.trackingStatusPill.em-producao{color:#ffb86b}.trackingStatusPill.saiu-entrega{color:#72b7ff}.trackingStatusPill.entregue{color:#65e6a1}.trackingStatusPill.cancelado{color:#ff737d}.trackingProgress{display:grid;grid-template-columns:repeat(4,1fr);gap:6px;padding:16px 0}.trackingProgressStep{text-align:center;opacity:.42}.trackingProgressStep.done{opacity:1}.trackingProgressStep span{display:flex;align-items:center;justify-content:center;width:27px;height:27px;margin:0 auto 5px;border-radius:50%;background:rgba(255,255,255,.08);font-size:11px;font-weight:900}.trackingProgressStep.done span{background:#e50914;color:#fff}.trackingProgressStep small{font-size:9px;font-weight:800}.trackingCardSection{padding-top:4px}.trackingItems{margin-top:8px}.trackingCardTotal{display:flex;justify-content:space-between;align-items:center;margin-top:14px;padding-top:13px;border-top:1px solid rgba(255,255,255,.08)}.trackingCardTotal strong{font-size:20px}.trackingPayment{display:flex;justify-content:space-between;gap:10px;margin-top:8px;font-size:12px}.trackingPix{margin-top:14px;padding:14px;border-radius:14px;text-align:center}.trackingPix.waiting{border:1px solid rgba(0,200,120,.22);background:rgba(0,120,80,.08)}.trackingPix.paid{border:1px solid rgba(0,200,120,.25);background:rgba(0,160,90,.12)}.trackingPix b,.trackingPix small{display:block}.trackingPix small{margin-top:4px}.trackingPix img{width:min(220px,70vw);display:block;margin:12px auto;border-radius:12px;background:#fff;padding:7px}.trackingPix button{width:100%;margin-top:8px;padding:11px;border:0;border-radius:10px;background:#e50914;color:#fff;font-weight:900}.trackingResult{min-width:0}.trackingResult .bvOrderItemGroup{padding:7px 0}@media(max-width:760px){.trackingGrid{grid-template-columns:1fr}.trackingStatusPill{font-size:10px;max-width:48%;white-space:normal;text-align:center}.trackingProgressStep small{font-size:8px}}</style>