...
(()=>{ 
  ...
  window.addToCart=id=>{const p=(window.products||[]).find(x=>String(x.id)===String(id));if(!p)return toast('Produto não encontrado.');const isRefrigerante=String(p.category||'')==='Bebidas'&&/coca|refri|refrigerante/i.test(String(p.name||''));const stock=Math.max(0,Number(p.stock)||0);if(isRefrigerante&&stock<=0)return toast('Este refrigerante está esgotado.');const r=window.cart.find(x=>String(x.id)===String(id));if(r)r.q=(Number(r.q)||0)+1;else window.cart.push({id:p.id,name:p.name,price:Number(p.price)||0,q:1});saveCart();toast('Produto adicionado ao pedido.')};
