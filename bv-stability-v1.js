...
(()=>{ 
  ...
  window.BV_STABILITY_VERSION='2026.09.24.9';
  ...
  window.renderDashboard=()=>{
    ...
    const list=$('dashboardOrders');if(list)list.innerHTML=done.slice(0,10).map(o=>`<div class="line"><span><b>#${esc(window.orderLabel(o))} · ${esc(o.customer||'')}</b><small>${esc(o.items||'')}</small></span><strong>${money(o.total)}</strong></div>`).join('')||'<p class="muted">Nenhum pedido entregue ainda.</p>';
  };
  ...
  window.renderProductsAdmin=()=>{const b=$('manage');if(!b)return;b.innerHTML=(window.products||[]).filter(x=>x.active!==false).map(x=>`<div class="line"><span><b>${esc(x.name)}</b><small>${money(x.price)} · ${esc(x.category)}</small></span><button type="button" onclick="removeProduct('${esc(x.id)}')">Excluir</button></div>`).join('')||'<p class="muted">Nenhum produto cadastrado.</p>'};
  ...
  window.BV_REFRESH_ORDERS=async()=>{
    ...
    if(role==='motoboy')q=q.or('motoboy_id.is.null,motoboy_id.eq.'+user.id).eq('status','em_producao');else if(role!=='administrador')q=q.eq('user_id',user.id);
  ...
  window.renderMotoOrders=async()=>{
    ...
    if(!sb)return;
    b.innerHTML='<div class="emptyState"><span>⏳</span><b>Buscando pedidos...</b><small>Atualizando pedidos em produção.</small></div>';
    try{
      const {data:{user}}=await sb.auth.getUser();if(!user)return;
      const r=await sb.from('orders').select('id,order_number,customer_name,phone,address,neighborhood,delivery_fee,total,payment_method,payment_status,status,created_at,motoboy_id').eq('status','em_producao').or('motoboy_id.is.null,motoboy_id.eq.'+user.id).order('created_at',{ascending:false});
  ...
  setInterval(()=>{if(window.BV_ROLE&&sb&&document.visibilityState!=='hidden')window.BV_REFRESH_ORDERS().catch(()=>{})},15000);
})();