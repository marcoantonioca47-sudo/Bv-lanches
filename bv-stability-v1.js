...
window.saveCfg=async()=>{if(!sb)return;const f=Number(String($('feeCfg')?.value||0).replace(',','.')),w=$('waCfg')?.value.trim()||'';if(!Number.isFinite(f)||f<0)return toast('Taxa padrão inválida.');const r=await sb.from('settings').upsert({id:1,fee:f,whatsapp:w},{onConflict:'id'});if(r.error)return toast('Erro ao salvar configurações: '+r.error.message);window.BV_DEFAULT_FEE=f;toast('Configurações salvas.')};
  ...
  window.BV_OPEN_PROMOTION=async promotionId=>{
    try{
      showPage('cardapio');
      await window.setMenuCategory?.('Promocoes',document.querySelector('#page-cardapio .menuCategoryTabs button[data-menu-category="Promocoes"]'));
      const card=document.querySelector('.promotionCatalogCard[data-promotion-id="'+CSS.escape(String(promotionId))+'"]');
      if(card){requestAnimationFrame(()=>card.scrollIntoView({behavior:'smooth',block:'center'}));card.classList.add('bvPromotionFocus');setTimeout(()=>card.classList.remove('bvPromotionFocus'),1800);}
    }catch(e){console.warn('[BV PROMO CLICK]',e);showPage('cardapio');}
  };
