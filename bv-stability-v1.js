...
(()=>{ 
  ...
  window.generatePromotionImage=async promotionId=>{try{const promo=(window.promotions||[]).find(x=>String(x.id)===String(promotionId));if(!promo)throw new Error('Promoção não encontrada.');const items=window.promotionItems?.[promo.id]||[];const ps=window.products||[];const rows=items.length?items:(promo.product_id?[{product_id:promo.product_id,quantity:1}]:[]);const dataRows=rows.map(i=>{const p=ps.find(x=>String(x.id)===String(i.product_id));return{p,q:Math.max(1,Number(i.quantity)||1)}}).filter(x=>x.p);if(!dataRows.length)throw new Error('A promoção não possui produtos válidos.');const esc=v=>String(v??'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&apos;');const wrap=(v,max)=>{const s=String(v||'');return s.length>max?s.slice(0,max-1)+'…':s};toast('🖼️ Montando imagem com os produtos e quantidades...');const count=dataRows.length;const cardW=count===1?430:count===2?360:300;const gap=25;const total=count*cardW+(count-1)*gap;const start=(1200-total)/2;const cards=dataRows.map((x,k)=>{const p=x.p;const img=p.image_url?'<image href="'+esc(p.image_url)+'" x="'+(start+k*(cardW+gap)+12)+'" y="145" width="'+(cardW-24)+'" height="230" preserveAspectRatio="xMidYMid slice"/>':'<rect x="'+(start+k*(cardW+gap)+12)+'" y="145" width="'+(cardW-24)+'" height="230" rx="18" fill="#2a2a2a"/><text x="'+(start+k*(cardW+gap)+cardW/2)+'" y="275" text-anchor="middle" fill="#aaa" font-size="52">🍔</text>';return'<rect x="'+(start+k*(cardW+gap))+'" y="133" width="'+cardW+'" height="350" rx="24" fill="#fff"/>'+img+'<rect x="'+(start+k*(cardW+gap)+22)+'" y="160" width="78" height="48" rx="20" fill="#ff3030"/><text x="'+(start+k*(cardW+gap)+61)+'" y="193" text-anchor="middle" fill="#fff" font-family="Arial" font-size="25" font-weight="900">'+x.q+'x</text><text x="'+(start+k*(cardW+gap)+cardW/2)+'" y="415" text-anchor="middle" fill="#151515" font-family="Arial" font-size="24" font-weight="900">'+esc(wrap(p.name,22))+'</text><text x="'+(start+k*(cardW+gap)+cardW/2)+'" y="451" text-anchor="middle" fill="#666" font-family="Arial" font-size="17">Quantidade: '+x.q+'</text>'}).join('');const svg='<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="630" viewBox="0 0 1200 630"><defs><linearGradient id="bg" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#100000"/><stop offset=".55" stop-color="#4a0000"/><stop offset="1" stop-color="#080808"/></linearGradient></defs><rect width="1200" height="630" fill="url(#bg)"/><text x="600" y="55" text-anchor="middle" fill="#ffd400" font-family="Arial" font-size="30" font-weight="900">BV LANCHES</text><text x="600" y="105" text-anchor="middle" fill="#fff" font-family="Arial" font-size="48" font-weight="900">'+esc(wrap(promo.name||'PROMOÇÃO',28))+'</text>'+cards+'<text x="600" y="535" text-anchor="middle" fill="#ff3030" font-family="Arial" font-size="54" font-weight="900">'+esc(money(Number(promo.promotional_price)||0))+'</text><text x="600" y="575" text-anchor="middle" fill="#ffd400" font-family="Arial" font-size="22" font-weight="900">OFERTA ESPECIAL • PEÇA AGORA</text></svg>';const data='data:image/svg+xml;charset=UTF-8,'+encodeURIComponent(svg);promo.image_url=data;const upd=await sb.from('promotions').update({image_url:data}).eq('id',promo.id);if(upd.error)throw upd.error;window.renderHomePromoBanner?.();window.renderPromotionsAdmin?.();toast('✅ Imagem criada com '+dataRows.reduce((s,x)=>s+x.q,0)+' produto(s), respeitando as quantidades.');return true}catch(e){console.error('[BV PROMO IMAGE]',e);toast(e?.message||'Não foi possível criar a imagem.');return false}};
