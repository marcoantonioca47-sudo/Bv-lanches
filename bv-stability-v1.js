...
(()=>{ 
  ...
  window.generatePromotionImage=async promotionId=>{try{const promo=(window.promotions||[]).find(x=>String(x.id)===String(promotionId));if(!promo)throw new Error('Promoção não encontrada.');if(window.BV_REFRESH_PROMOTIONS)await window.BV_REFRESH_PROMOTIONS();const items=window.promotionItems?.[promo.id]||[];const ps=window.products||[];const rows=items.length?items:(promo.product_id?[{product_id:promo.product_id,quantity:1}]:[]);const base=rows.map(i=>({p:ps.find(x=>String(x.id)===String(i.product_id)),q:Math.max(1,parseInt(i.quantity,10)||1)})).filter(x=>x.p);if(!base.length)throw new Error('Não encontrei os produtos desta promoção.');const units=[];base.forEach(x=>{for(let i=0;i<x.q;i++)units.push(x.p)});toast('🖼️ Criando uma nova imagem com '+units.length+' produto(s)...');const loadImg=url=>new Promise(resolve=>{if(!url)return resolve(null);const im=new Image();im.crossOrigin='anonymous';im.onload=()=>resolve(im);im.onerror=()=>resolve(null);im.src=url});const imgs=await Promise.all(units.map(p=>loadImg(p.image_url)));const c=document.createElement('canvas');c.width=1200;c.height=630;const ctx=c.getContext('2d');const bg=ctx.createLinearGradient(0,0,1200,630);bg.addColorStop(0,'#120000');bg.addColorStop(.5,'#520000');bg.addColorStop(1,'#080808');ctx.fillStyle=bg;ctx.fillRect(0,0,1200,630);ctx.textAlign='center';ctx.fillStyle='#ffd400';ctx.font='900 30px Arial';ctx.fillText('BV LANCHES',600,42);ctx.fillStyle='#fff';ctx.font='900 42px Arial';ctx.fillText(String(promo.name||'PROMOÇÃO').slice(0,30),600,88);const maxCards=Math.min(units.length,8),gap=14,cols=maxCards<=3?maxCards:4,rowsN=Math.ceil(maxCards/cols),cardW=(1120-gap*(cols-1))/cols,cardH=rowsN===1?340:205,startX=(1200-(cardW*cols+gap*(cols-1)))/2,startY=108;for(let k=0;k<maxCards;k++){const col=k%cols,row=Math.floor(k/cols),x=startX+col*(cardW+gap),y=startY+row*(cardH+gap);ctx.fillStyle='#fff';ctx.beginPath();ctx.roundRect(x,y,cardW,cardH,18);ctx.fill();ctx.save();ctx.beginPath();ctx.roundRect(x+7,y+7,cardW-14,cardH-58,14);ctx.clip();const im=imgs[k];if(im){const scale=Math.max((cardW-14)/im.width,(cardH-65)/im.height),w=im.width*scale,h=im.height*scale;ctx.drawImage(im,x+7+(cardW-14-w)/2,y+7+(cardH-65-h)/2,w,h)}else{ctx.fillStyle='#eee';ctx.fillRect(x+7,y+7,cardW-14,cardH-65);ctx.font='42px Arial';ctx.fillStyle='#777';ctx.fillText('🍔',x+cardW/2,y+(cardH-65)/2+15)}ctx.restore();ctx.fillStyle='#e40000';ctx.beginPath();ctx.roundRect(x+12,y+12,58,38,18);ctx.fill();ctx.fillStyle='#fff';ctx.font='900 19px Arial';ctx.fillText('1x',x+41,y+38);ctx.fillStyle='#151515';ctx.font='900 '+(cardW<280?'17':'20')+'px Arial';ctx.fillText(String(units[k].name||'Produto').slice(0,22),x+cardW/2,y+cardH-25)}ctx.fillStyle='#ff3030';ctx.font='900 48px Arial';ctx.fillText(money(Number(promo.promotional_price)||0),600,rowsN===1?505:565);ctx.fillStyle='#ffd400';ctx.font='900 20px Arial';ctx.fillText('OFERTA ESPECIAL • PEÇA AGORA',600,600);const data=c.toDataURL('image/jpeg',.9);const upd=await sb.from('promotions').update({image_url:data}).eq('id',promo.id);if(upd.error)throw upd.error;promo.image_url=data;window.renderHomePromoBanner?.();window.renderPromotionsAdmin?.();toast('✅ Nova imagem criada com '+units.length+' unidade(s), repetindo cada produto conforme a quantidade.');return true}catch(e){console.error('[BV PROMO IMAGE]',e);toast('❌ '+(e?.message||'Não foi possível criar a imagem.'));return false}};
