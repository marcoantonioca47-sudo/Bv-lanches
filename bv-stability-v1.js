...
(()=>{ 
  ...
  window.renderProducts=()=>{const b=$('products');if(!b)return;const now=Date.now(),ps=window.products||[];if(window.BV_MENU_CATEGORY==='Promocoes'){const active=(window.promotions||[]).filter(x=>x.active&&(!x.starts_at||new Date(x.starts_at).getTime()<=now)&&(!x.ends_at||new Date(x.ends_at).getTime()>=now));b.innerHTML=active.length?active.map(x=>{const items=window.promotionItems?.[x.id]||[],legacy=x.product_id?[{product_id:x.product_id,quantity:1}]:[],list=(items.length?items:legacy).map(i=>{const p=ps.find(y=>String(y.id)===String(i.product_id));return (i.quantity||1)+'x '+(p?.name||'Produto')}).join(' • ');return '<article class="productCard promotionCatalogCard"><div class="productImage"><span>🏷️</span></div><div class="promoCatalogBody"><h3>'+esc(x.name)+'</h3><p>'+esc(x.description||'Oferta especial do BV Lanches')+'</p><div class="promotionCatalogItems">'+esc(list)+'</div><div class="promotionCatalogPrice"><del>'+money(x.original_price||0)+'</del><strong>'+money(x.promotional_price||0)+'</strong></div><button type="button" class="promoCatalogButton" onclick="addPromotionToCart(\\''+esc(x.id)+'\\')">Adicionar promoção</button></div></article>'}).join(''):'<div class="emptyState"><span>🏷️</span><b>Nenhuma promoção ativa.</b><small>As promoções aparecerão aqui enquanto estiverem ativas.</small></div>';return;}b.innerHTML='';const cat=window.BV_CAT||'Lanches';const a=ps.filter(p=>p.active!==false&&String(p.category||'Lanches')===cat);const activePromos=(window.promotions||[]).filter(x=>x.active&&(!x.starts_at||new Date(x.starts_at).getTime()<=now)&&(!x.ends_at||new Date(x.ends_at).getTime()>=now));b.innerHTML=a.length?a.map(p=>{const singles=activePromos.filter(x=>{const items=window.promotionItems?.[x.id]||[];const rows=items.length?items:(x.product_id?[{product_id:x.product_id,quantity:1}]:[]);return rows.length===1&&String(rows[0].product_id)===String(p.id)&&Math.max(1,Number(rows[0].quantity)||1)===1&&Number(x.promotional_price)<Number(p.price)}).sort((u,v)=>Number(u.promotional_price)-Number(v.promotional_price);const promo=singles[0]||null;const price=promo?Number(promo.promotional_price):Number(p.price)||0;const fallback=p.category==='Bebidas'?'🥤':'🍔';const media=p.image_url?'<img src="'+esc(p.image_url)+'" alt="'+esc(p.name)+'" loading="lazy" decoding="async" onerror="this.style.display=\\'none\\';this.nextElementSibling.style.display=\\'grid\\'"><span style="display:none">'+fallback+'</span>':'<span>'+fallback+'</span>';const priceHtml=promo?'<div class="promoPrice"><del>'+money(p.price)+'</del><b>'+money(price)+'</b></div>':'<b>'+money(price)+'</b>';return '<article class="productCard product">'+(promo?'<div class="promoBadge">🔥 PROMOÇÃO</div>':'')+'<div class="productImage">'+media+'</div><div class="productInfo"><h3>'+esc(p.name)+'</h3><p>'+esc(p.description||'')+'</p><div class="productBottom">'+priceHtml+'<button type="button" onclick="addToCart(\\''+esc(p.id)+'\\')">+ Adicionar</button></div></div></article>'}).join(''):'<div class="panel"><p class="muted">Nenhum produto disponível nesta categoria.</p></div>';};
