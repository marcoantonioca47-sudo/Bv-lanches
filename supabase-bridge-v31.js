...
(function(){
  ...
  window.refreshDeliveryFeeForNeighborhood=async function(name){const raw=String(name||'').trim();if(!raw)return null;const norm=v=>String(v||'').trim().toLowerCase().normalize('NFD').replace(/[\\u0300-\\u036f]/g,'').replace(/\\s+/g,' ');try{const {data,error}=await sb.from('neighborhood_fees').select('name,fee,active').eq('active',true);if(error||!Array.isArray(data))return null;const hit=data.find(x=>norm(x.name)===norm(raw));return hit?Number(hit.fee)||0:null}catch(e){console.warn('Taxa do bairro:',e);return null}};window.refreshDeliveryFees=async function(){try{return await feesDB()}catch(e){console.warn('Taxas:',e);return false}};
