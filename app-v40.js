...
window.BV_APP_VERSION='2026.09.23.7';
...
function applyAccess(){document.querySelectorAll('.adminOnly').forEach(x=>x.style.display=admin()?'':'none');document.querySelectorAll('.adminHide').forEach(x=>x.style.display=admin()?'none':'');document.querySelectorAll('[data-role="motoboyOnly"]').forEach(x=>x.style.display=moto()&&!admin()?'':'none');const q=$('adminQuick');if(q)q.style.display=admin()?'':'none'}
function allowed(p){return admin()||publicPages.includes(p)||(moto()&&p==='pedidos')}
function showPage(p){if(!pages[p])p='inicio';if(!allowed(p)){toast('Você não tem permissão para acessar esta tela.');return false}localStorage.bv_page=p;document.querySelectorAll('.page').forEach(x=>x.classList.remove('activePage'));const x=$('page-'+p);if(x)x.classList.add('activePage');document.querySelectorAll('.sideNav button').forEach(b=>b.classList.toggle('active',b.dataset.page===p));if($('pageTitle'))$('pageTitle').textContent=pages[p];if($('sidebar'))$('sidebar').classList.remove('open');try{if(p==='cardapio')renderProducts();if(p==='pedido')renderCart();if(p==='produtos')renderManage();if(admin()||moto())renderAdmin();if(p==='config'){if(typeof window.renderUsers==='function')window.renderUsers();else renderUsers();renderBairroFees()};if(p==='acompanhar')renderTracking()}catch(e){console.error('BV tela:',e)}return true}
function toggleSidebar(){if($('sidebar'))$('sidebar').classList.toggle('open')}function closeLogin(){if($('login'))$('login').style.display='none';if($('err'))$('err').textContent=''}function openAdmin(p='dashboard'){if(admin()||(moto()&&p==='pedidos'))showPage(p);else{if($('login'))$('login').style.display='flex';if($('email'))$('email').focus()}}
...
function renderProducts(){const box=$('products');if(box)box.innerHTML=products.map(p=>`<article class="product"><div class="photo">${p.emoji||'🍔'}</div><span class="categoryBadge">${p.category||'Lanches'}</span><h3>${p.name}</h3><p>${p.desc||''}</p><div class="price">${brl(p.price)}</div><button class="add" onclick="add(${p.id})">Adicionar</button></article>`).join('');renderManage()}
function add(id){const p=products.find(x=>x.id===id);if(!p)return;const i=cart.find(x=>x.id===id);i?i.q++:cart.push({...p,q:1});localStorage.bv_cart=JSON.stringify(cart);renderCart();toast('Produto adicionado')}
...
function addBairroFee(){const n=$('bairroCfg')?.value.trim().toLowerCase(),v=parseFloat($('bairroFeeCfg')?.value);if(!n||isNaN(v)||v<0)return toast('Informe bairro e taxa.');config.bairroFees[n]=v;localStorage.bv_config=JSON.stringify(config);$('bairroCfg').value='';$('bairroFeeCfg').value='';renderBairroFees();toast('Bairro cadastrado.')}
function renderBairroFees(){const b=$('bairroFees');if(!b)return;b.innerHTML=Object.entries(config.bairroFees||{}).map(([n,v])=>`<div class="line"><span>${n}</span><b>${brl(v)}</b></div>`).join('')||'<p class="muted">Nenhum bairro cadastrado.</p>'}
...
(function(){
  ...
  function renderFilteredOrders(){
    ...
    box.innerHTML=list.map(o=>{
      ...
      return '<div class="order"><div class="line"><b>#'+esc(o.id)+' — '+esc(o.customer)+'</b><span class="status">'+esc(o.status)+'</span></div><p><b>Itens:</b> '+esc(o.items)+'</p><p>📍 '+adr+'</p><p>💳 '+esc(o.payment)+(o.paid?' · Pago':'')+'</p><div class="orderMeta"><span>'+esc(o.phone||'Sem telefone')+'</span><span>'+brl(o.total)+'</span></div><select onchange="statusOrder(\''+String(o.id).replace(/'/g,"\\'")+'\',this.value)">'+opts+'</select>'+(o.payment==='Pix'&&!o.paid?'<button class="confirmPix" onclick="confirmPix(\''+String(o.id).replace(/'/g,"\\'")+'\')">✅ Confirmar Pix</button>':'')+'</div>';
    }).join('');
  }
