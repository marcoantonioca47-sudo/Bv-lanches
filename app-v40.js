...
window.BV_APP_VERSION='2026.09.23.26';
...
function applyAccess(){const isAdmin=admin(),isMoto=moto()&&!isAdmin;document.querySelectorAll('.adminOnly').forEach(x=>x.style.display=isAdmin?'':'none');document.querySelectorAll('.adminHide').forEach(x=>x.style.display=isAdmin?'none':'');document.querySelectorAll('[data-role="motoboyOnly"]').forEach(x=>x.style.display=isMoto?'':'none');document.querySelectorAll('.sideNav button[data-page]').forEach(x=>x.style.display=isMoto?(x.dataset.page==='pedidos'?'':'none'):'');document.querySelectorAll('.motoboyHide').forEach(x=>x.style.display=isMoto?'none':'');const q=$('adminQuick');if(q)q.style.display=isAdmin?'':'none';if(isMoto){localStorage.bv_page='pedidos';document.querySelectorAll('.page').forEach(x=>x.classList.remove('activePage'));const pg=$('page-pedidos');if(pg)pg.classList.add('activePage');document.querySelectorAll('.sideNav button').forEach(b=>b.classList.toggle('active',b.dataset.page==='pedidos'));if($('pageTitle'))$('pageTitle').textContent='Pedidos'}}
