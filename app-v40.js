...
window.BV_APP_VERSION='2026.09.23.42';
const usersDefault=[{id:'admin-marco',name:'Marco',email:'marco@bvlanches.com',pass:'01022005',role:'administrador'},{id:'user-demo',name:'Cliente',email:'cliente@bv.com',pass:'01022005',role:'motoboy'}];function users(){let u=safeParse('bv_users',null);if(!Array.isArray(u))u=[];let changed=false;const oldAdmin=u.findIndex(x=>String(x.email||'').trim().toLowerCase()==='admin@bvlanches.com');const newAdmin=u.findIndex(x=>String(x.email||'').trim().toLowerCase()==='marco@bvlanches.com');if(oldAdmin>=0&&newAdmin<0){u[oldAdmin]={...u[oldAdmin],id:'admin-marco',name:'Marco',email:'marco@bvlanches.com',pass:'01022005',role:'administrador'};changed=true}else if(oldAdmin>=0&&newAdmin>=0){u.splice(oldAdmin,1);changed=true}usersDefault.forEach(d=>{const i=u.findIndex(x=>String(x.email||'').trim().toLowerCase()===d.email);if(i<0){u.push({...d});changed=true}else{if(!u[i].id){u[i].id=d.id;changed=true}if(String(d.email).toLowerCase()==='cliente@bv.com'&&u[i].role!=='motoboy'){u[i].role='motoboy';changed=true}else if(!u[i].role){u[i].role=d.role;changed=true}}});if(changed||!localStorage.getItem('bv_users'))localStorage.bv_users=JSON.stringify(u);return u}function normalizeRole(v){const r=String(v||'').trim().toLowerCase();return r==='admin'||r==='administrador'||r==='administrator'?'administrador':r==='motoboy'||r==='motoboy'?'motoboy':r==='usuario'||r==='user'?'usuario':''}
function role(){const r=normalizeRole(sessionStorage.getItem('bv_role'));if(r&&r!==sessionStorage.getItem('bv_role'))sessionStorage.setItem('bv_role',r);return r}
...
function init(){const nameBox=$('loggedUserName');if(nameBox)nameBox.textContent=sessionStorage.getItem('bv_user_name')||'';applyAccess();const r=role();if(r)showPage(r==='administrador'?'dashboard':r==='motoboy'?'pedidos':'inicio');else showPage('inicio');setTimeout(applyAccess,0);setTimeout(applyAccess,250);setTimeout(applyAccess,1000);if(saved){if($('name'))$('name').value=saved.name||'';if($('phone'))$('phone').value=saved.phone||'';if($('street'))$('street').value=saved.rua||'';if($('num'))$('num').value=saved.numero||'';if($('bairro'))$('bairro').value=saved.bairro||'';if($('cep'))$('cep').value=saved.cep||'';if($('comp'))$('comp').value=saved.complemento||''}}
