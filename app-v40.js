...
function add(id){const key=String(id);const p=products.find(x=>String(x.id)===key);if(!p)return toast('Produto não encontrado. Atualize o cardápio.');const i=cart.find(x=>String(x.id)===key);i?i.q++:cart.push({...p,q:1});localStorage.bv_cart=JSON.stringify(cart);renderCart();toast('Produto adicionado')}
