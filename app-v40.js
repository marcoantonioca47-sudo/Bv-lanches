...
window.BV_APP_VERSION='2026.09.23.19';
const usersDefault=[{id:'admin-marco',name:'Marco',email:'marco@bvlanches.com',pass:'01022005',role:'administrador'},{id:'user-demo',name:'Cliente',email:'cliente@bv.com',pass:'01022005',role:'usuario'}];function users(){let u=safeParse('bv_users',null);if(!Array.isArray(u))u=[];let changed=false;const oldAdmin=u.findIndex(x=>String(x.email||'').trim().toLowerCase()==='admin@bvlanches.com');const newAdmin=u.findIndex(x=>String(x.email||'').trim().toLowerCase()==='marco@bvlanches.com');if(oldAdmin>=0&&newAdmin<0){u[oldAdmin]={...u[oldAdmin],id:'admin-marco',name:'Marco',email:'marco@bvlanches.com',pass:'01022005',role:'administrador'};changed=true}else if(oldAdmin>=0&&newAdmin>=0){u.splice(oldAdmin,1);changed=true}usersDefault.forEach(d=>{const i=u.findIndex(x=>String(x.email||'').trim().toLowerCase()===d.email);if(i<0){u.push({...d});changed=true}else{if(!u[i].id){u[i].id=d.id;changed=true}if(!u[i].role){u[i].role=d.role;changed=true}}});if(changed||!localStorage.getItem('bv_users'))localStorage.bv_users=JSON.stringify(u);return u}function role(){return sessionStorage.getItem('bv_role')||''}function admin(){return role()==='administrador'}function moto(){return role()==='motoboy'}
...
function applyAccess(){const motoMode=moto()&&!admin();document.querySelectorAll('.adminOnly').forEach(x=>x.style.display=admin()?'':'none');document.querySelectorAll('.adminHide').forEach(x=>x.style.display=admin()?'none':'');document.querySelectorAll('[data-role="motoboyOnly"]').forEach(x=>x.style.display=motoMode?'':'none');document.querySelectorAll('.sideNav button[data-page]').forEach(x=>x.style.display=motoMode?(x.dataset.page==='pedidos'?'':'none'):'');const q=$('adminQuick');if(q)q.style.display=admin()?'':'none';if(motoMode&&localStorage.bv_page!=='pedidos')localStorage.bv_page='pedidos'}
function allowed(p){return moto()&&!admin()?p==='pedidos':(admin()||publicPages.includes(p))}
